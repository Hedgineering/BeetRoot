...
const register = (req, res) => {
  ...
  if(!req.body.username || !req.body.email || !req.body.firstName || !req.body.lastName || !req.body.status) {
  ...
  userModel.findOne({username: req.body.username}, (err, user) => {
    ...
    if(requestedRoleStrings.includes("Admin")) {
      res.status(400).json({message: "Admin role cannot be assigned to a user"});
      return;
    }
    ...
    bcrypt.hash(req.body.password, saltRounds, function(err, hashedPassword) {
      ...
      const roles = roleModel.find({}).select({name: 1, _id: 1}).then(async (roles) => {
        ...
        if (userRolesSet.size < requestedRoleStrings.length) {
          res.status(400).json({ message: "Roles must be valid. You have too many types of roles!" });
          return;
        }
        for(let role of requestedRoleStrings) {
          console.log('checking role: ' + role);
          if (!userRolesSet.has(role)) {
            res.status(400).json({ message: "Roles must be valid. Incorrect roles requested." });
            return;
          }
        }
        ...
        requestedRoles = roles.filter((role) => requestedRoleStrings.has(role.name)).map((role) => role._id);
